...
namespace HoloToolkit.Unity.SpatialMapping
{
    ...
    public class TapToPlace : MonoBehaviour, IInputClickHandler
    {
        ...
        protected virtual void Update()
        {
            ...
            // collider so it sits properly on surfaces. //Removed by Seosamh because it messes up where the grid is placed

            if (PlaceParentOnTap)
            {
                var x = ParentGameObjectToPlace.transform.position + (placementPosition);
                var y = ParentGameObjectToPlace.transform.position + (placementPosition - gameObject.transform.position);
                placementPosition = y;// - gameObject.transform.position);
